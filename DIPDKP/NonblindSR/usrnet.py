...
def torch_rfft(input, signal_ndim=2, normalized=False, onesided=False):
    output_fft_new = torch.fft.fft2(input, dim=(-2, -1))
    output_fft_new_2dim = torch.stack((output_fft_new.real, output_fft_new.imag), -1)
    return output_fft_new_2dim


def torch_irfft(output_fft_new_2dim, signal_ndim=2, normalized=False, onesided=False):
    output_ifft_new = torch.fft.ifft2(torch.complex(output_fft_new_2dim[..., 0],
                                                    output_fft_new_2dim[..., 1]), dim=(-2, -1))
    output_ifft_new = output_ifft_new.real

    return output_ifft_new
...
def p2o(psf, shape):
    ...
    otf = torch_rfft(otf, 2, onesided=False)
...
class DataNet(nn.Module):
    ...
    def forward(self, x, FB, FBC, F2B, FBFy, alpha, sf):
        FR = FBFy + torch_rfft(alpha*x, 2, onesided=False)
        x1 = cmul(FB, FR)
        ...
        Xest = torch_irfft(FX, 2, onesided=False)
...
class USRNet(nn.Module):
    ...
    def forward(self, x, k, sf, sigma):
        ...
        FBFy = cmul(FBC, torch_rfft(STy, 2, onesided=False))
